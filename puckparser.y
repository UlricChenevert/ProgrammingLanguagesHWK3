...
WhileStatement : K_WHILE Expression K_DO StatementSequence K_END { cout << "RULE: WhileStatement ::= WHILE Expression DO StatementSequence END" << endl; } ;
...
Factor : T_INTEGER     { cout << "RULE: Factor ::= T_INTEGER" << endl; }
       ...
       | T_STRING     { cout << "RULE: Factor ::= T_STRING" << endl; }
       | T_IDENT        { cout << "RULE: Factor ::= T_IDENT" << endl; }
       | K_LPAREN Expression K_RPAREN { cout << "RULE: Factor ::= K_LPAREN Expression K_RPAREN" << endl; }
       | OP_NEG Factor       { cout << "RULE: Factor ::= OP_NEG Factor" << endl; } ;

Term : Factor               { cout << "RULE: Term ::= Factor" << endl; }
     | Factor OP_MULT Term  { cout << "RULE: Term ::= Factor OP_MULT Term" << endl; } ;

SimpleExpression : Term                          { cout << "RULE: SimpleExpression ::= Term" << endl; }
                 | Term OP_ADD SimpleExpression  { cout << "RULE: SimpleExpression ::= Term OP_ADD SimpleExpression" << endl; }

Expression  : SimpleExpression { cout << "RULE: Expression ::= SimpleExpression" << endl; } 
            | SimpleExpression OP_RELATION Expression { cout << "RULE: Expression ::= SimpleExpression OP_RELATION Expression" << endl; } 
